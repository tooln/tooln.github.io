...
### Install GO:
```
wget https://go.dev/dl/go1.24.8.linux-amd64.tar.gz -O go.tar.gz && rm -rf /usr/local/go && tar -C /usr/local -xzf go.tar.gz
```
```
rm -f go.tar.gz && rm -f zsh.sh && source ~/.zshrc && go version
```
